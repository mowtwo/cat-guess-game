...
const DEFAULT_ITEMS: Item[] = [
  {
    ...
    name: "纸巾",
    image:
      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect x="20" y="40" width="60" height="40" fill="%23eee" stroke="%23ccc" stroke-width="2"/><path d="M30 40 Q50 10 70 40" fill="%23fff" stroke="%23ddd"/></svg>',
  },
  {
    id: "default_2",
    name: "萝卜",
    image:
      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="M50 90 Q20 40 50 40 Q80 40 50 90" fill="orange"/><path d="M50 40 L40 10 M50 40 L50 5 M50 40 L60 10" stroke="green" stroke-width="3"/></svg>',
  },
];
...
export default function App() {
  ...
  const [catMessage, setCatMessage] =
    useState<string>("人类，快选一个东西让我猜！");
  ...
  const startCatGuessing = (userTarget: Item) => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      setCatChoice(chosen);
      itemRefs.current[chosen.id]?.scrollIntoView({
        behavior: "smooth",
        block: "nearest",
        inline: "center",
      });
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (err.message === "MISSING_KEY" || err.message === "API_ERROR") {
          setErrorMsg(
            "AI 识别失败。请点击右上角设置图标检查 Gemini API Key 喵！"
          );
  ...
  const saveNewItem = () => {
    if (newItemName && newItemImage) {
      setItems((prev) => [
        ...prev,
        { id: Date.now().toString(), name: newItemName, image: newItemImage },
      ]);
    ...
    <div className="flex flex-col bg-orange-50 selection:bg-orange-200 min-h-screen font-sans text-gray-800">
      ...
      <header className="top-0 z-50 sticky flex justify-between items-center bg-white/80 shadow-sm backdrop-blur-md p-4 border-orange-100 border-b">
        <div className="flex items-center gap-3">
          <div className="bg-orange-500 shadow-lg shadow-orange-200 p-2 rounded-xl">
            <Sparkles className="text-white" size={20} />
          </div>
          <h1 className="font-black text-orange-900 text-xl tracking-tight">
            小猫猜猜乐
          </h1>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-orange-100 shadow-inner px-4 py-1.5 rounded-full font-bold text-orange-800">
            <Trophy size={16} /> <span>{score}</span>
          </div>
          <button
      ...
      <main className="flex flex-col flex-1 justify-center items-center gap-8 mx-auto p-4 w-full max-w-5xl">
        ...
        <div className="relative flex flex-col justify-center items-center w-full min-h-[300px]">
          <div className="relative flex justify-center items-center w-56 h-56 transition-all duration-300">
            <svg
              viewBox="0 0 240 240"
              className={`w-full h-full drop-shadow-2xl ${gameState === "guessing" ? "animate-bounce" : ""
                }`}
            >
              <path
                d="M60 70 L40 20 L100 50 Z"
                fill="#FDBA74"
                stroke="#442c1d"
                strokeWidth="3"
              />
              <path
                d="M180 70 L200 20 L140 50 Z"
                fill="#333"
                stroke="#442c1d"
                strokeWidth="3"
              />
              ...
              />
              <path
                d="M50 100 Q40 120 50 160 Q80 140 70 100 Z"
                fill="#FDBA74"
                opacity="0.8"
              />
              <path
                d="M190 100 Q200 120 190 160 Q160 140 170 100 Z"
                fill="#333"
                opacity="0.8"
              />
              <path
                d="M70 75 Q120 45 170 75 L175 90 Q120 75 65 90 Z"
                fill="#3b82f6"
                stroke="#1e3a8a"
                strokeWidth="2"
              />
              <path
                d="M120 45 L125 55 L135 55 L127 62 L130 72 L120 65 L110 72 L113 62 L105 55 L115 55 Z"
                fill="#facc15"
              />
              {catEmotion === "thinking" ? (
                <>
                  <circle cx="95" cy="115" r="7" fill="#333" />
                  <circle cx="145" cy="115" r="7" fill="#333" />
                  <path
                ...
                <>
                  <path
                    d="M85 115 Q95 105 105 115"
                    fill="none"
                    stroke="#333"
                    strokeWidth="3"
                  />
                  <path
                    d="M135 115 Q145 105 155 115"
                    fill="none"
                    stroke="#333"
                    strokeWidth="3"
                  />
                ...
                <>
                  <circle cx="95" cy="120" r="10" fill="#333" />
                  <circle cx="145" cy="120" r="10" fill="#333" />
                  <circle cx="98" cy="116" r="4" fill="white" />
                  <circle cx="148" cy="116" r="4" fill="white" />
                </>
              )}
              <path d="M115 135 L125 135 L120 142 Z" fill="#fda4af" />
              <path
                d="M120 142 Q110 150 100 145 M120 142 Q130 150 140 145"
                fill="none"
                stroke="#333"
                strokeWidth="2"
              />
            ...
            <div
              className={`absolute -top-12 left-1/2 -translate-x-1/2 w-48 bg-white border-2 border-orange-200 rounded-2xl p-2 shadow-lg text-xs font-bold text-center transition-opacity duration-500 ${isAiTalking ? "opacity-100" : "opacity-0"
                }`}
            >
              {catMessage}
          ...
          {gameState === "fail" && (
            <div className="slide-in-from-bottom-4 z-50 flex gap-4 mt-8 animate-in duration-500 fade-in">
              <button
                onClick={() => targetItem && startCatGuessing(targetItem)}
                className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 shadow-xl px-8 py-3 rounded-2xl font-bold text-white active:scale-95 transition-all transform"
              >
              ...
              <button
                ...
                className="bg-white hover:bg-gray-50 shadow-lg px-8 py-3 border border-gray-100 rounded-2xl font-bold text-gray-600 transition-all"
              >
                换个物品
              </button>
            </div>
        ...
        <div className="relative w-full">
          <div className="flex justify-between items-end mb-4 px-4">
            <div>
              <h2 className="font-black text-orange-900 text-lg">物品仓库</h2>
            ...
            <button
              ...
              className="bg-white hover:bg-orange-50 shadow-lg p-3 border border-orange-50 rounded-2xl text-orange-500 hover:scale-110 active:scale-90 transition-all"
            >
              <Plus size={24} />
            </button>
          </div>
          ...
          <div className="relative">
            {/* 指示爪子 */}
            <div
              className={`absolute pointer-events-none transition-all duration-300 ease-out z-[60] ${gameState === "guessing"
                ? "opacity-100 scale-100"
                : "opacity-0 scale-50"
                }`}
              style={{
                left:
                  catChoice && itemRefs.current[catChoice.id]
                    ? itemRefs.current[catChoice.id]!.offsetLeft + 64
                    : "50%",
                top: "-10px",
            ...
            >
              <svg width="80" height="80" viewBox="0 0 100 100">
                <path
                  d="M50 90 C30 90 20 70 20 50 C20 30 35 20 50 20 C65 20 80 30 80 50 C80 70 70 90 50 90"
                  fill="white"
                  stroke="#442c1d"
                  strokeWidth="3"
                />
                <path
                  d="M50 90 C30 90 20 70 20 50 L35 50 Q50 65 65 50 L80 50 C80 70 70 90 50 90"
                  fill="#FDBA74"
                />
                <circle cx="35" cy="40" r="10" fill="#fda4af" />
                <circle cx="50" cy="30" r="10" fill="#fda4af" />
                <circle cx="65" cy="40" r="10" fill="#fda4af" />
                <ellipse cx="50" cy="65" rx="18" ry="15" fill="#fda4af" />
              </svg>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`group relative flex-shrink-0 w-32 h-44 flex flex-col items-center justify-between p-4 rounded-3xl border-2 transition-all duration-300 snap-center drag-item
                      ${isCatThinking && gameState === "guessing"
                        ? "border-orange-400 bg-white scale-110 z-20 shadow-2xl shadow-orange-200"
                        : isSelected
                          ? "border-orange-500 bg-orange-50 ring-4 ring-orange-100/50 z-10"
                  ...
                  >
                    ...
                    <span className="mt-2 w-full font-black text-orange-900 text-sm text-center truncate pointer-events-none">
                      {item.name}
                    </span>
                  </div>
                );
        ...
        <div className="z-[120] fixed inset-0 flex justify-center items-center bg-orange-900/40 backdrop-blur-md p-6">
          <div className="relative bg-white shadow-2xl p-8 rounded-[2rem] w-full max-w-sm">
            <div className="flex justify-between items-center mb-6">
              <h3 className="flex items-center gap-2 font-black text-orange-950 text-xl">
                <Settings className="text-orange-500" size={20} /> 开发者设置
              </h3>
              <button
                onClick={() => setIsSettingsOpen(false)}
                className="bg-orange-50 p-2 rounded-full text-orange-500"
              >
                <X size={20} />
              </button>
            </div>
            <div className="space-y-4">
              <div className="flex gap-3 bg-blue-50 p-4 rounded-2xl text-blue-700 text-xs leading-relaxed">
                ...
                <p>
                  AI 物品识别需要 <b>Gemini API Key</b>。你可以从 Google AI
                  Studio 免费获取。
                </p>
              </div>
              <div className="space-y-2">
                <label className="flex items-center gap-1 ml-1 font-bold text-orange-400 text-xs">
      ...
      {isAddModalOpen && (
        <div className="z-[100] fixed inset-0 flex justify-center items-center bg-orange-900/40 backdrop-blur-md p-6">
          <div className="relative bg-white shadow-2xl p-8 rounded-[2rem] w-full max-w-md overflow-hidden">
            ...
            {errorMsg && (
              <div
                className={`mb-4 p-4 rounded-xl flex items-center gap-2 text-sm font-bold border animate-shake ${errorMsg.includes("API Key")
            ...
            <div className="space-y-6">
              <div className="space-y-1">
                <label className="ml-2 font-bold text-orange-400 text-xs">
                  物品名称
                </label>
                <input
                  type="text"
                  ...
                  placeholder={
                    isAiAnalyzing ? "猫咪正在观察..." : "给它起个名字喵？"
                  }
                  className="bg-orange-50 px-6 py-4 rounded-2xl outline-none ring-orange-200 focus:ring-2 w-full font-bold transition-all"
                />
              </div>
              <div className="space-y-1">
                <label className="ml-2 font-bold text-orange-400 text-xs">
                ...
                <div
                  className={`border-3 border-dashed rounded-[1.5rem] p-8 text-center cursor-pointer transition-all ${newItemImage
                    ? "border-orange-200"
                ...
                >
                  {isAiAnalyzing ? (
                    ...
                    <img
                      src={newItemImage}
                      className="mx-auto rounded-xl w-40 h-40 object-contain"
                    />
                  ) : (
                    <div className="py-6">
                      <Upload
              ...
              <button
                onClick={saveNewItem}
                disabled={!newItemName || !newItemImage || isAiAnalyzing}
                className={`w-full font-black py-5 rounded-2xl shadow-lg transition-all ${!newItemName || !newItemImage || isAiAnalyzing
